...
const MiniMap = () => {
  ...
  const handleDrag = (e: PointerEvent, info: PanInfo) => {
    ...
    const newCameraX = camera.x - info.delta.x * divider;
    const newCameraY = camera.y - info.delta.y * divider;
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className="border-2 border-yellow-600 relative flex items-center justify-center"
          ref={minimapRef}
          style={{
            width: CANVAS_SIZE / divider,
            height: CANVAS_SIZE / divider,
          }}
          onPointerEnter={() => setInBounds(true)}
          onPointerLeave={() => setInBounds(false)}
        >
          <motion.div
            drag
            onDrag={handleDrag}
            ...
            dragMomentum={false}
            dragElastic={0}
            className="bg-white absolute border-2 cursor-all-scroll border-gray-700"
            style={{
              width: width / divider,
